...
const KitchenProvider = ({ children }) => {
  ...
  const fetchStatus = async () => {
    const res = await axios.get("https://nescafe-ovhf.onrender.com/api/settings");
    if (res.data.success) {
  ...
  const toggleKitchen = async () => {
    const res = await axios.post("https://nescafe-ovhf.onrender.com/api/settings/toggle-kitchen");
    if (res.data.success) {
      setKitchenOpen(res.data.kitchenOpen);
    }
  };
